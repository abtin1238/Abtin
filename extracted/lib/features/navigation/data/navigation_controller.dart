...
class NavigationController extends StateNotifier<NavigationState> {
  ...
  void _checkHazards(LatLng pos, double speedKmh) {
    ...
    // ---- تابلوی هشدارِ زیرِ نوارِ بالا: فقط دوربینِ کنترلِ سرعت و سرعت‌گیر ----
    HazardBannerType? bannerType;
    switch (alert.hazard.type) {
      case 'speed_camera':
        bannerType = HazardBannerType.speedCamera;
        break;
      case 'speed_bump':
        bannerType = HazardBannerType.speedBump;
        break;
      default:
        bannerType = null;
    }
    if (bannerType != null) {
      _hazardBannerTimer?.cancel();
      state = state.copyWith(
        hazardBanner: HazardBannerInfo(
          type: bannerType,
          title: alert.hazard.title,
          distanceMeters: alert.distanceMeters,
        ),
      );
      _hazardBannerTimer = Timer(const Duration(seconds: 7), () {
        if (mounted) state = state.copyWith(clearHazardBanner: true);
      });
    }
  }
