...
class NavigationController extends StateNotifier<NavigationState> {
  ...
  void _checkHazards(LatLng pos, double speedKmh) {
    ...
    // محدودیتِ سرعت از دیتای نقشه (هر نوع هشدار) به‌روزرسانی می‌شود.
    if (alert.hazard.speedLimit != null) {
      state = state.copyWith(speedLimitKmh: alert.hazard.speedLimit);
    }

    // نوارِ هشدارِ بالا **فقط** دوربینِ کنترلِ سرعت و سرعت‌گیر را نشان می‌دهد.
    final t = alert.hazard.type;
    if (t == 'speed_camera' || t == 'speed_bump') {
      unawaited(_voice.announceHazard(alert.hazard.title));
      state = state.copyWith(hazardType: t, hazardTitle: alert.hazard.title);
      _hazardClearTimer?.cancel();
      _hazardClearTimer = Timer(const Duration(seconds: 6), () {
        if (mounted) state = state.copyWith(clearHazard: true);
      });
    }
  }
