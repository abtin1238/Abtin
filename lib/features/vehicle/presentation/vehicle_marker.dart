...
class _VehicleMarkerState extends State<VehicleMarker> {
  ...
  Widget build(BuildContext context) {
    // نکته‌ی مهم (ریشه‌ی واقعی باگ «پیکان نمایش داده نمی‌شود»):
    // قبلاً اگر toScreenLocation حتی یک‌بار هم موفق نمی‌شد (مثلاً به‌خاطر
    // تأخیر در آماده شدن کامل استایل نقشه یا مشکل پلاگین)، build() برای
    // همیشه SizedBox.shrink برمی‌گرداند و پیکان اصلاً دیده نمی‌شد. حالا در
    // نبود مختصات دقیق، به‌جای مخفی‌شدن، پیکان روی مرکز صفحه (جایی که در
    // حالت دنبال‌کردن دوربین، خودرو معمولاً همان‌جاست) نمایش داده می‌شود و
    // هم‌زمان تلاش برای گرفتن مختصات دقیق در پس‌زمینه ادامه پیدا می‌کند.
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final left = s != null
            ...
            : constraints.maxWidth / 2 - markerSize / 2;
        final top = s != null
            ? s.y.toDouble() - markerSize / 2
            : constraints.maxHeight / 2 - markerSize / 2;
        return Positioned(
          left: left,
          top: top,
          child: IgnorePointer(
